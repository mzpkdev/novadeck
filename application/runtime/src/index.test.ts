...
describe("distributed runtime CLI", () => {
  context("using its built entrypoint and public built client", () => {
    it("loads .env, authenticates, runs a real shell, and persists only metadata after a process restart", async ({
      resources,
    }) => {
      ...
      expect(
        (await fetch(`${runtime.origin}/api/status`, { signal: AbortSignal.timeout(10_000) }))
          .status,
      ).toBe(200)
      ...
      await expect(
        connection.sessions.list({ projectId: project.id }, restartCall),
  ...
  context.skipIf(process.platform === "win32")(
    "on POSIX, where Node handles SIGTERM gracefully",
    () => {
      it("exits successfully and terminates its live owned shell on SIGTERM", async ({
        resources,
      }) => {
        const directory = await directoryFixture(resources)
        const runtime = await launchCli(directory, resources)
